...
namespace Ragnarok
{
    ...
    public partial class Login_Form : Form
    {
        ...
        private void Login_Form_Load(object sender, EventArgs e)
        {
            ...
            LocalIP_Box.Text = CFG.Local_IP;
            LocalPort_Box.Text = CFG.Local_Port.ToString();
        ...
        private void Save_botton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var ipcheck = LocalIP_Box.Text.Split('.');
                foreach (var i in ipcheck)
                {
                    if (Convert.ToInt32(i)>=0 && Convert.ToInt32(i)<=255);
                    else throw new FormatException();
                }
                CFG.Local_IP = LocalIP_Box.Text;

                int portcheck = Convert.ToInt32(LocalPort_Box.Text);
                if (portcheck>=0 && portcheck <= 65535)
                {
                    CFG.Local_Port = portcheck;
                }
                else throw new FormatException();

                ipcheck = ServerIP_Box.Text.Split('.');
                foreach (var i in ipcheck)
                {
                    if (Convert.ToInt32(i) >= 0 && Convert.ToInt32(i) <= 255) ;
                    else throw new FormatException();
                ...
                portcheck = Convert.ToInt32(ServerPort_Box.Text);
                if (portcheck >= 0 && portcheck <= 65535)
                {
        ...
        public void ConnectToServer()
        {
            ...
            try
            {
                ...
                PUB.s.Connect(CFG.Server_IP, CFG.Server_Port);
                PUB.s.Send(Encoding.UTF8.GetBytes(CFG.UserID));
                int tmp = PUB.s.Receive(bytesReceived, bytesReceived.Length, 0);
                Invoke(chlable, Encoding.UTF8.GetString(bytesReceived, 0, tmp));

                PUB.s.Send(Encoding.UTF8.GetBytes(CFG.TargetID));
